...
public class GetDataEddy extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String userAgent =  request.getHeader("user-agent");
			...
			try {
				...
				String queryStr = "select a.tair,a.co2,a.wind,a.wind_ang,a.co2_flux, " +
							"a.data from dati_eddy a, mobile_stations b " +
				...
				while(dsm.next()){
					...
					if(dsm.getDouble(3)<0){
						jobj.put("wind", 0.0);
					}else{
						jobj.put("wind", dsm.getDouble(3));
					}
					
					if(dsm.getDouble(4)<0){
						jobj.put("wind_ang", 0.0);
					}else{
						jobj.put("wind_ang", dsm.getDouble(4));
					}
					
					if(dsm.getDouble(5)<0){
						jobj.put("co2_flux", 0.0);
					}else{
						jobj.put("co2_flux", dsm.getDouble(5));
					}
