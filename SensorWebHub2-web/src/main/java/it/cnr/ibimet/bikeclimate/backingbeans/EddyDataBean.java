...
import it.cnr.ibimet.bikeclimate.entities.EddyData;
import it.cnr.ibimet.bikeclimate.entities.EddyStatistics;
...
@ManagedBean(name="eddyDataBean")
@SessionScoped
public class EddyDataBean implements Serializable {
	...
	private List<EddyStatistics> eddyStatisticsList;
	private EddyStatistics eddyStatisticsSelected;
	private Date data1, data2;
	private String formatoRadio;
	private boolean consenso;
	private Date fromData;
	private Date toData;
	private List<EddyData> eddyDataList;
	private EddyData eddyDataSelected;
	...
	public List<EddyStatistics> getEddyStatisticsList() {
		return eddyStatisticsList;
	}

	public void setEddyStatisticsList(List<EddyStatistics> eddyStatisticsList) {
		this.eddyStatisticsList = eddyStatisticsList;
	}
	
	public EddyStatistics getEddyStatisticsSelected() {
		return eddyStatisticsSelected;
	}

	public void setEddyStatisticsSelected(EddyStatistics eddyStatisticsSelected) {
		this.eddyStatisticsSelected = eddyStatisticsSelected;
	}
	...
	public List<EddyData> getEddyDataList() {
		return eddyDataList;
	}

	public void setEddyDataList(List<EddyData> eddyDataList) {
		this.eddyDataList = eddyDataList;
	}

	public EddyData getEddyDataSelected() {
		return eddyDataSelected;
	}

	public void setEddyDataSelected(EddyData eddyDataSelected) {
		this.eddyDataSelected = eddyDataSelected;
	}

	@PostConstruct
	public void init(){
		System.out.println("EddyData - init");
		consenso=false;
		statodownload=true;
	...
	//	createFile();
		...
		try {
                    ...
                    TableSchema tSchema = new TableSchema(dsm, "dati_eddy");
                        
                    System.out.println("Ecco la stazione "+eddyStationBean.getEddyStationSelezionato2().getId_bike_sensor());
                    ChartParams cp = new ChartParams(dsm, eddyStationBean.getEddyStationSelezionato2().getId_bike_sensor(), loginBean.getLingua());
	...
	System.out.println("ecco: "+selectStr);
			...
			dsm.setParameter(ParameterType.INT, ""+eddyStationBean.getEddyStationSelezionato2().getId_bike_sensor(), 1);
		...
		//	bikeDataList = new ArrayList<BikeData>();
         ...
         public void populate(){
			 graphConsistencyBean.setThisMobile3(eddyStationBean.getEddyStationSelezionato().getId_bike_sensor());
			 graphConsistencyBean.doCons4Eddy();
		...
		try {
			...
			dsm = new TDBManager("jdbc/urbandb");
				...
				//giardino=idgiardini.get(0);
                        ...
                        System.out.println("Ecco la stazione "+ eddyStationBean.getEddyStationSelezionato().getId_bike_sensor());
                        
                        ChartParams cp = new ChartParams(dsm,eddyStationBean.getEddyStationSelezionato().getId_bike_sensor(),loginBean.getLingua());
			...
			String sqlString="select "+selectStr+"b.nome as nome,a.data as data "
					+ "from dati_eddy a, mobile_stations b " 
			...
			dsm.setParameter(ParameterType.INT, ""+eddyStationBean.getEddyStationSelezionato().getId_bike_sensor(), 1);
			...
			while(dsm.next()){
                      ...
                      //      System.out.println("Ecco i numeri di campi: "+cp.getParamNumbers());
                            ...
                            for(int i=0; i<cp.getParamNumbers(); i++){
                                ...
                                String CType = tSchema.getColumnType(cp.getParam().get(i));
                             //   System.out.println("analizzo questo:  "+cp.getParam().get(i) +"-"+CType);
